// <copyright file="CliSteps.cs" company="PlaceholderCompany">
...
public static class CliSteps
{
        ...
        => async s =>
        {
            ...
            if (!string.IsNullOrWhiteSpace(raw))
            {
                foreach (var part in raw.Split('|', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries))
                {
                    ...
                    {
                        root = Path.GetFullPath(part.Substring(5));
                    }
                    ...
                    else if (part.StartsWith("exclude=", StringComparison.OrdinalIgnoreCase))
                    {
                        excludeDirs.AddRange(part.Substring(8).Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries));
                    }
                    else if (part.StartsWith("pattern=", StringComparison.OrdinalIgnoreCase))
                    {
                        patterns.AddRange(part.Substring(8).Split(';', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries));
                    }
                    else if (part.StartsWith("max=", StringComparison.OrdinalIgnoreCase) && long.TryParse(part.AsSpan(4), out var m))
                    {
                        maxBytes = m;
                    }
                    else if (part.Equals("norec", StringComparison.OrdinalIgnoreCase))
                    {
                        recursive = false;
                    }
                }
            ...
            try
            {
                ...
                var store = s.Branch.Store ?? new TrackedVectorStore();
        ...
        => async s =>
        {
            ...
            if (!string.IsNullOrWhiteSpace(raw))
            {
                foreach (var part in raw.Split('|', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries))
                {
                    ...
                    else if (part.StartsWith("addEvery=", StringComparison.OrdinalIgnoreCase) && int.TryParse(part.AsSpan(9), out var ae) && ae > 0)
                    {
                        addEvery = ae;
                    }
                }
            ...
            try
            {
                ...
                foreach (var doc in docs)
                {
                    if (string.IsNullOrWhiteSpace(doc.PageContent)) { fileIndex++;
                        continue; }
        ...
        => async s =>
        {
            try
            {
                ...
                if (sourceEvent is not null)
                {
                    ...
                    if (parts.Length == 3 && Directory.Exists(parts[2]))
                    {
                        root = parts[2];
                    }
                }
                ...
                var vectors = await Pipeline.Ingestion.SolutionIngestion.IngestAsync(
                    s.Branch.Store ?? new LangChainPipeline.Domain.Vectors.TrackedVectorStore(),
                    root,
                    s.Embed,
                    opts);
    ...
    public static Step<CliPipelineState, CliPipelineState> UseDraft(string? args = null)
        => async s =>
        {
            ...
            if (s.Trace)
            {
                Console.WriteLine("[trace] Draft produced");
            }
    ...
    public static Step<CliPipelineState, CliPipelineState> UseCritique(string? args = null)
        => async s =>
        {
            ...
            if (s.Trace)
            {
                Console.WriteLine("[trace] Critique produced");
            }
    ...
    public static Step<CliPipelineState, CliPipelineState> UseImprove(string? args = null)
        => async s =>
        {
            ...
            if (s.Trace)
            {
                Console.WriteLine("[trace] Improvement produced");
            }
        ...
        => s =>
        {
            if (!string.IsNullOrWhiteSpace(args))
            {
                ...
                if (m.Success && int.TryParse(m.Groups[1].Value, out var k))
                {
                    ...
                    if (s.Trace)
                    {
                        Console.WriteLine($"[trace] RetrievalK set to {k}");
                    }
                }
            }
    ...
    public static Step<CliPipelineState, CliPipelineState> ZipStream(string? args = null)
        => async s =>
        {
            ...
            if (raw.Contains('|'))
            {
                ...
                {
                    if (mod.StartsWith("batch=", StringComparison.OrdinalIgnoreCase) && int.TryParse(mod.AsSpan(6), out var bs) && bs > 0)
                    {
                        batchSize = bs;
                    }
                    else if (mod.Equals("noText", StringComparison.OrdinalIgnoreCase))
                    {
                        includeXmlText = false;
                    }
                    else if (mod.Equals("noEmbed", StringComparison.OrdinalIgnoreCase))
            ...
            var full = Path.GetFullPath(path);
            if (!File.Exists(full)) { s.Branch = s.Branch.WithIngestEvent($"zip:missing:{full}", Array.Empty<string>());
                return s; }
    ...
    public static Step<CliPipelineState, CliPipelineState> EmbedZip(string? args = null)
        => async s =>
        {
            ...
            for (int i = 0; i < pendingIds.Count; i += batchSize)
            {
                ...
                {
                    if (DeferredZipTextCache.TryTake(id, out var original) && !string.IsNullOrWhiteSpace(original))
                    {
                        return original;
                    }
    ...
    public static Step<CliPipelineState, CliPipelineState> LlmStep(string? args = null)
        => async s =>
        {
            ...
            try
            {
                ...
                if (s.Trace)
                {
                    Console.WriteLine("[trace] LLM output length=" + text.Length);
                }
            }
            catch (Exception ex)
            {
    ...
    public static Step<CliPipelineState, CliPipelineState> DivideAndConquerRag(string? args = null)
        => async s =>
        {
            ...
            if (!string.IsNullOrWhiteSpace(raw))
            {
                foreach (var part in raw.Split('|', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries))
                {
                    if (part.StartsWith("k=", StringComparison.OrdinalIgnoreCase) && int.TryParse(part.AsSpan(2), out var kv) && kv > 0)
                    {
                        k = kv;
                    }
                    else if (part.StartsWith("group=", StringComparison.OrdinalIgnoreCase) && int.TryParse(part.AsSpan(6), out var gv) && gv > 0)
                    {
                        group = gv;
                    }
                    else if (part.StartsWith("sep=", StringComparison.OrdinalIgnoreCase))
                    {
                        sep = part.Substring(4).Replace("\\n", "\n");
                    }
                    else if (part.StartsWith("template=", StringComparison.OrdinalIgnoreCase))
                    {
                        template = part.Substring(9);
                    }
                    else if (part.StartsWith("final=", StringComparison.OrdinalIgnoreCase))
                    {
                        finalTemplate = part.Substring(6);
                    }
                    else if (part.Equals("stream", StringComparison.OrdinalIgnoreCase))
                    {
                        streamPartials = true;
                    }
            ...
            if (s.Retrieved.Count == 0)
            {
                try
                {
                    s = await RetrieveSimilarDocuments($"amount={k}")(s);
                }
                catch
                { /* ignore */
                }
            ...
            try
            {
                ...
                if (s.Trace)
                {
                    Console.WriteLine($"[trace] DCRAG final length={s.Output.Length}");
                }
    ...
    public static Step<CliPipelineState, CliPipelineState> DecomposeAndAggregateRag(string? args = null)
        => async s =>
        {
            ...
            if (!string.IsNullOrWhiteSpace(raw))
            {
                foreach (var part in raw.Split('|', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries))
                {
                    if (part.StartsWith("subs=", StringComparison.OrdinalIgnoreCase) && int.TryParse(part.AsSpan(5), out var sv) && sv > 0)
                    {
                        subs = sv;
                    }
                    else if (part.StartsWith("per=", StringComparison.OrdinalIgnoreCase) && int.TryParse(part.AsSpan(4), out var pv) && pv > 0)
                    {
                        per = pv;
                    }
                    else if (part.StartsWith("k=", StringComparison.OrdinalIgnoreCase) && int.TryParse(part.AsSpan(2), out var kv) && kv > 0)
                    {
                        k = kv;
                    }
                    else if (part.StartsWith("sep=", StringComparison.OrdinalIgnoreCase))
                    ...
                    else if (part.StartsWith("decompose=", StringComparison.OrdinalIgnoreCase))
                    {
                        decomposeTpl = part.Substring(10);
                    }
                    else if (part.StartsWith("template=", StringComparison.OrdinalIgnoreCase))
                    {
                        subTpl = part.Substring(9);
                    }
                    else if (part.StartsWith("final=", StringComparison.OrdinalIgnoreCase))
                    {
                        finalTpl = part.Substring(6);
                    }
                }
            }
            ...
            try
            {
                s = await RetrieveSimilarDocuments($"amount={k}|query={question.Replace("|", ":")}")(s);
            }
            catch
            { /* ignore */
            }
            ...
            try
            {
                ...
                if (s.Trace)
                {
                    Console.WriteLine($"[trace] DARAG final length={s.Output.Length}");
                }
        ...
        => async s =>
        {
            ...
            if (!string.IsNullOrWhiteSpace(raw))
            {
                foreach (var part in raw.Split('|', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries))
                {
                    ...
                    {
                        newModel = part.Substring(6);
                    }
                    else if (part.StartsWith("embed=", StringComparison.OrdinalIgnoreCase))
                    {
                        newEmbed = part.Substring(6);
                    }
                    else if (part.Equals("remote", StringComparison.OrdinalIgnoreCase))
                    {
                        forceRemote = true;
                    }
                }
            ...
            if (string.IsNullOrWhiteSpace(newModel) && string.IsNullOrWhiteSpace(newEmbed))
            {
            ...
            if (model == null && !string.IsNullOrWhiteSpace(newModel))
            {
                ...
                if (newModel == "deepseek-coder:33b")
                {
                    oc.Settings = OllamaPresets.DeepSeekCoder33B;
                }
    ...
    public static Step<CliPipelineState, CliPipelineState> UseExternalChain(string? args = null)
        => async s =>
        {
            ...
            string? name = null;
            string[] inKeys = Array.Empty<string>();
            string[] outKeys = ["Output"];
            bool trace = false;
            foreach (var part in raw.Split('|', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries))
            {
                ...
                else if (part.StartsWith("in=", StringComparison.OrdinalIgnoreCase))
                {
                    inKeys = part.Substring(3).Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
                }
                else if (part.StartsWith("out=", StringComparison.OrdinalIgnoreCase))
                {
                    outKeys = part.Substring(4).Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
                }
                else if (part.Equals("trace", StringComparison.OrdinalIgnoreCase))
                {
                    trace = true;
                }
            }

            if (string.IsNullOrWhiteSpace(name)) { s.Branch = s.Branch.WithIngestEvent("chain:error:no-name", Array.Empty<string>());
                return s; }
            ...
            try
            {
                ...
                if (call is null)
                {
                    s.Branch = s.Branch.WithIngestEvent($"chain:error:no-call:{name}", Array.Empty<string>());
                    return s;
                }
                ...
                if (trace)
                {
                    Console.WriteLine($"[chain] {name} export={dict?.Count ?? 0}");
                }
                ...
                if (valueProp?.GetValue(valuesObj) is IDictionary<string, object?> dict2)
                {
                    dict = dict2;
                }
                ...
                if (trace)
                {
                    Console.WriteLine($"[chain] {name} import keys={string.Join(',', outKeys)}");
                }
            }
            catch (Exception ex)
            {
        ...
        int count)
    {
        ...
        if (state.Branch.Store is not TrackedVectorStore store)
        {
            return blocks;
        }
    ...
    public static Step<CliPipelineState, CliPipelineState> LangChainRetrieveStep(string? args = null)
        => async s =>
        {
            ...
            if (s.Branch.Store is not IVectorCollection vectorCollection)
            {
                if (s.Trace)
                {
                    Console.WriteLine("[trace] LangChainRetrieve: store is not IVectorCollection");
                }
            ...
            if (string.IsNullOrWhiteSpace(query))
            {
                if (s.Trace)
                {
                    Console.WriteLine("[trace] LangChainRetrieve: no query text");
                }
            ...
            try
            {
                ...
                if (s.Trace)
                {
                    Console.WriteLine($"[trace] LangChainRetrieve: retrieved {s.Retrieved.Count} documents");
                }
            }
            catch (Exception ex)
            {
                if (s.Trace)
                {
                    Console.WriteLine($"[trace] LangChainRetrieve: error {ex.Message}");
                }
            }
    ...
    public static Step<CliPipelineState, CliPipelineState> LangChainCombineStep(string? args = null)
        => async s =>
        {
            ...
            if (docs.Count == 0)
            {
                if (s.Trace)
                {
                    Console.WriteLine("[trace] LangChainCombine: no documents to combine");
                }
            ...
            if (result.Value.TryGetValue(outputKey, out var context))
            {
                ...
                if (s.Trace)
                {
                    Console.WriteLine($"[trace] LangChainCombine: combined context length={s.Context.Length}");
                }
            }
    ...
    public static Step<CliPipelineState, CliPipelineState> LangChainTemplateStep(string? args = null)
        => async s =>
        {
            ...
            if (string.IsNullOrWhiteSpace(templateText))
            {
                if (s.Trace)
                {
                    Console.WriteLine("[trace] LangChainTemplate: no template provided");
                }
            ...
            if (result.Value.TryGetValue("text", out var output))
            {
                ...
                if (s.Trace)
                {
                    Console.WriteLine($"[trace] LangChainTemplate: formatted prompt length={s.Prompt.Length}");
                }
            }
    ...
    public static Step<CliPipelineState, CliPipelineState> LangChainLlmStep(string? args = null)
        => async s =>
        {
            ...
            if (llmField == null)
            {
                if (s.Trace)
                {
                    Console.WriteLine("[trace] LangChainLLM: cannot access underlying chat model");
                }
            ...
            if (chatModel == null)
            {
                if (s.Trace)
                {
                    Console.WriteLine("[trace] LangChainLLM: chat model is null");
                }
            ...
            if (result.Value.TryGetValue(outputKey, out var output))
            {
                ...
                if (s.Trace)
                {
                    Console.WriteLine($"[trace] LangChainLLM: output length={s.Output.Length}");
                }
            }
...
Helpful Answer:";
            ...
            if (string.IsNullOrWhiteSpace(question))
            {
                if (s.Trace)
                {
                    Console.WriteLine("[trace] LangChainRAG: no question provided");
                }
