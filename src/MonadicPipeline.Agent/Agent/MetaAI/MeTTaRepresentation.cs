// <copyright file="MeTTaRepresentation.cs" company="PlaceholderCompany">
...
public sealed class MeTTaRepresentation
{
    ...
    private readonly Dictionary<string, string> stateAtoms = new();
    ...
    public async Task<Result<Unit, string>> TranslatePlanAsync(Plan plan, CancellationToken ct = default)
    {
        try
        {
            ...
            sb.AppendLine($"(goal {planId} \"{this.EscapeMeTTa(plan.Goal)}\")");
            ...
            for (int i = 0; i < plan.Steps.Count; i++)
            {
                ...
                sb.AppendLine($"(step {planId} {stepId} {i} \"{this.EscapeMeTTa(step.Action)}\")");
                sb.AppendLine($"(expected {stepId} \"{this.EscapeMeTTa(step.ExpectedOutcome)}\")");
                ...
                foreach (var param in step.Parameters)
                {
                    ...
                    sb.AppendLine($"(param {stepId} \"{this.EscapeMeTTa(param.Key)}\" \"{this.EscapeMeTTa(value)}\")");
                }
            ...
            this.stateAtoms[plan.Goal] = planId;

            // Add all facts to MeTTa
            var factResult = await this.engine.AddFactAsync(sb.ToString(), ct);
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            for (int i = 0; i < execution.StepResults.Count; i++)
            {
                ...
                {
                    sb.AppendLine($"(error {resultId} \"{this.EscapeMeTTa(stepResult.Error)}\")");
                }
                ...
                foreach (var state in stepResult.ObservedState)
                {
                    ...
                    sb.AppendLine($"(observed {resultId} \"{this.EscapeMeTTa(state.Key)}\" \"{this.EscapeMeTTa(value)}\")");
                }
            }

            var result = await this.engine.AddFactAsync(sb.ToString(), ct);
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            foreach (var tool in tools.All)
            {
                ...
                sb.AppendLine($"(tool {toolId} \"{this.EscapeMeTTa(tool.Name)}\")");
                sb.AppendLine($"(tool-desc {toolId} \"{this.EscapeMeTTa(tool.Description)}\")");
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            var queryResult = await this.engine.ExecuteQueryAsync(query, ct);

            return queryResult.Match(
                success => this.ParseNextNodeCandidates(success),
    ...
    public async Task<Result<Unit, string>> AddConstraintAsync(
        ...
        CancellationToken ct = default)
    {
        var result = await this.engine.AddFactAsync(constraint, ct);
        return result.Match(
        ...
        CancellationToken ct = default)
    {
        var query = $@"!(match &self 
            (and 
                (goal $plan ""{this.EscapeMeTTa(goal)}"")
                (capability $tool $cap)
            )
            $tool)";

        var result = await this.engine.ExecuteQueryAsync(query, ct);

        return result.Match(
            success => Result<List<string>, string>.Success(this.ParseToolList(success)),
    ...
    private List<string> ParseToolList(string mettaOutput)
    {
        ...
        foreach (var line in lines)
        {
            ...
            {
                tools.Add(trimmed.Replace("tool_", string.Empty).Replace("-", "_"));
            }
        }
