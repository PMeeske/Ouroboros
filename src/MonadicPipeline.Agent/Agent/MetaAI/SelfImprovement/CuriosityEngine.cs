// <copyright file="CuriosityEngine.cs" company="PlaceholderCompany">
...
public sealed class CuriosityEngine : ICuriosityEngine
{
    ...
    private readonly CuriosityEngineConfig config;
    private readonly ConcurrentBag<(Plan plan, double novelty, DateTime when)> explorationHistory = new();
    private int totalExplorations = 0;
    private int sessionExplorations = 0;
        ...
        CuriosityEngineConfig? config = null)
    {
        ...
        this.config = config ?? new CuriosityEngineConfig();
    }
        ...
        CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            {
                var similarity = this.CalculateActionSimilarity(plan, exp.Plan);
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            var opportunities = await this.IdentifyExplorationOpportunitiesAsync(5, ct);
...
STEP 2: ...";
            ...
            // Parse plan
            var steps = this.ParseExploratorySteps(response);

            // Safety check all steps
            if (this.config.EnableSafeExploration)
            {
        ...
        CancellationToken ct = default)
    {
        ...
        if (this.sessionExplorations >= this.config.MaxExplorationPerSession)
        {
            return false;
        }
        ...
        var explorationProbability = 1.0 - this.config.ExploitationBias;

        // Increase exploration if we haven't explored much recently
        var recentExplorations = this.explorationHistory
        ...
        if (!string.IsNullOrWhiteSpace(currentGoal))
        {
            ...
            var novelty = await this.ComputeNoveltyAsync(goalPlan, ct);

            if (novelty > this.config.ExplorationThreshold)
            {
        ...
        CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            var experiences = await this.GetAllExperiences(ct);
...
";
    ...
    public void RecordExploration(Plan plan, ExecutionResult execution, double actualNovelty)
    {
        ...
        this.explorationHistory.Add((plan, actualNovelty, DateTime.UtcNow));
        this.totalExplorations++;
        this.sessionExplorations++;
    }
    ...
    public Dictionary<string, double> GetExplorationStats()
    {
        ...
        stats["total_explorations"] = this.totalExplorations;
        stats["session_explorations"] = this.sessionExplorations;

        var recent = this.explorationHistory
    ...
    private async Task<List<Experience>> GetAllExperiences(CancellationToken ct)
    {
        ...
        return await this.memory.RetrieveRelevantExperiencesAsync(query, ct);
    }
            ...
            if (trimmed.StartsWith("STEP"))
            {
                if (currentAction != null)
                {
                    steps.Add(new PlanStep(
                        currentAction,
                        new Dictionary<string, object> { ["expected_learning"] = currentExpected ?? string.Empty },
